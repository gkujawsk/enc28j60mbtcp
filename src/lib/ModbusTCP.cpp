...
void ModbusTCP::setServerIPAddress(const uint8_t* ipAddr)
{
  memcpy(serverIP, ipAddr, 4);
}
...
uint8_t ModbusTCP::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  // Serial.print("MODBUS TCP REQUEST ADU:");
  // for(byte i = 0; i < u8ModbusADUSize; i++) {
  //   Serial.printf("[0x%02X] ", u8ModbusADU[i]);
  // }
  // Serial.printf("\r\n");
  stash.prepare(u8ModbusADU, u8ModbusADUSize);

  byte session = ether.tcpSend();
  uint16_t rlen;

  uint32_t tcpTransactionTout = millis();
  const char* reply;
  while(true) {
    ether.packetLoop(ether.packetReceive());
    reply = ether.tcpReply(session, rlen);
    if(reply != 0) {
      break;
    }
    if(millis() - tcpTransactionTout > 1000) {
      return MBResponseTimedOut;
    }
  }

  // Serial.printlnf("Received %d B response: ", rlen);
  ...
  u32StartTime = millis();
  memcpy(u8ModbusADU, reply, rlen);
  while (u16BytesLeft && !u8MBStatus)
  {
    ether.packetLoop(ether.packetReceive());

      // u8ModbusADU[u8ModbusADUSize] = reply[u8ModbusADUSize];
      u8ModbusADUSize++;
      u16BytesLeft--;
  ...
  // Serial.print("MODBUS TCP RESPONSE ADU:");
  // for(byte i = 0; i < u8ModbusADUSize; i++) {
  //   Serial.printf("[0x%02X] ", u8ModbusADU[i]);
  // }
  // Serial.printf("\r\n");
